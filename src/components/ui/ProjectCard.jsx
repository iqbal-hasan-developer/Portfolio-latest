  ...
  return (
    <div className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300">
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-black via-black/60 to-transparent opacity-80 goup-hover:opacity-90 transition-opacity duration-300" />
      ...
      <div className="flex flex-wrap">
        {technologies.map((tech, index) => (
          <span key={index} className="px-3 py-1 rounded-md my-3 mx-2 text-xs font-medium text-primary bg-primary/10 border-primary/20 transition-colors duration-300">
            {tech}
          </span>
        ))}
      </div>
      ...
      {metrics && (
        <div className="flex items-center gap-2 py-5 border-t border-white/10 px-4">
